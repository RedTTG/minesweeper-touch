...
#ss = (1200, 600)
pe.display.make(ss, "Minesweeper touch", pe.display.DISPLAY_MODE_NORMAL)
# Size Settings
maxzoom = 4
minzoom = 0.4
rect = (10, 10)
...
def reloadData():
    ...
    for gamemode in presets['gamemodes']:
        res['gamemodeText'].append(pe.Text(gamemode['name'], "Resources/font.ttf", 20, (ss[0]/2, ss[1]/2+65), [ext['text'], None]))
    res['startGameText'] = pe.Text("New Game", "Resources/font.ttf", 20, (ss[0]/2, ss[1]/2+120), [ext['text'], None])
    res['startGameTextSelected'] = pe.Text("New Game", "Resources/font.ttf", 20, (ss[0]/2, ss[1]/2+120), [ext['background'], None])
    # Over edition
    res['gamemodeTextOver'] = []
    for gamemode in presets['gamemodes']:
        res['gamemodeTextOver'].append(pe.Text(gamemode['name'], "Resources/font.ttf", 20, (ss[0]-ss[0]/6, ss[1]/2+15), [ext['text'], None]))
    res['startGameTextOver'] = pe.Text("New Game", "Resources/font.ttf", 20, (ss[0]-ss[0]/6, ss[1]/2+120), [ext['text'], None])
    res['startGameTextSelectedOver'] = pe.Text("New Game", "Resources/font.ttf", 20, (ss[0]-ss[0]/6, ss[1]/2+120), [ext['background'], None])
    #
    res['beginGameText'] = pe.Text("Tap to begin.", "Resources/font.ttf", 20, (ss[0]/2, ss[1]/2), [ext['background'], None])
...
class touchButton:
    def image(rect, ic, ac, action=None):
        ...
        fingerRect = pe.Rect(fingerPosition[0]-10, fingerPosition[1]-10, 20, 20)
        buttonRect = pe.Rect(*rect)
...
lastSelect = (-1,-1)
lastfinger = (0,0)
fingerRect = pe.Rect(-10,-10,10,10)
pressMove = False
noZoomBefore = True
...
while run:
    ...
    if game_state == 'ingame' or game_state == 'pregameover' or game_state == 'gameover':
        ...
        visibleRect = pe.Rect(visibleRect[0],visibleRect[1],visibleRect[2] - visibleRect[0], visibleRect[3] - visibleRect[1])
        # Inner display
        ...
        with surface:
            ...
            for y in range(len(board)):
                for x in range(len(board[y])):
                    if not pe.Rect(x * 30, y * 30, 30, 30).colliderect(visibleRect):
                        continue
            ...
            while i < len(openings):
                ...
                if pe.Rect(*rectOpening).colliderect(visibleRect):
            ...
            if len(fingers) > 0:
                ...
                loc2 = (rect[0]+100, rect[1]+100)
            uiRect = pe.Rect(ss[0]/2 - 60, ss[1] - ss[1]/10 - 31, 120, 62)
        ...
        tempSurface.blit(surface.surface, (0, 0), visibleRect)
        ...
        if debug:
            ...
            pe.draw.rect(pe.colors.red, (10 + (visibleRect[0]*scalex)/10, 10 + (visibleRect[1]*scaley)/10, (visibleRect[2]*scalex)/10, (visibleRect[3]*scaley)/10), 2)
        ...
        if debug and zooming:
            try:
                pe.draw.line(pe.colors.blue, worldtoscreen(*zoom_start_pos), worldtoscreen(afterzoomx, zoom_start_pos[1]),1)
                pe.draw.line(pe.colors.blue, worldtoscreen(afterzoomx, zoom_start_pos[1]), worldtoscreen(afterzoomx, afterzoomy),1)
        ...
        if pe.Rect(*pe.mouse.pos(), 1, 1).colliderect(pe.Rect((ss[0]/2)-presets['buttonSpace']-70, (ss[1]/2)+100, presets['buttonSpace']*2+100, 40)):
            ...
            pe.draw.circle(ext['text'], ((ss[0] / 2) + presets['buttonSpace'] + 30, (ss[1] / 2) + 120), 20, presets['buttonLineSize'])
            pe.draw.rect(ext['background'],((ss[0]/2)-presets['buttonSpace']-30, (ss[1]/2)+100, presets['buttonSpace']*2+60, 40), 0)
    ...
    if game_state == 'pregameover':
        ...
        if pe.Rect(*pe.mouse.pos(), 1, 1).colliderect(pe.Rect((ss[0]-ss[0]/6)-presets['buttonSpace']-70, (ss[1]/2)+100, presets['buttonSpace']*2+100, 40)):
